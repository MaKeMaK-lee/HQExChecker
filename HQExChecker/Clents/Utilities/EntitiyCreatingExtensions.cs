...
namespace HQExChecker.Clents.Utilities
{
    public static class EntitiyCreatingExtensions
    {
        ...
        {
            decimal openPrice = Decimal(innerArray[1]);
            decimal closePrice = Decimal(innerArray[2]);
            decimal highPrice = Decimal(innerArray[3]);
            decimal lowPrice = Decimal(innerArray[4]);
